...
purrr::walk(
  ...
  .f = function(i) {
    ...
    current_html <- fs::path("_lau_nuts_concordance_by_geo_html", current_filename)
    
    if (fs::file_exists(current_html)==FALSE) {
      rmarkdown::render(input = "_process_lau_nuts_concordance_by_geo.Rmd",
                        params = list(
                          lau_year = current_combo_df$lau_year,
                          nuts_year = current_combo_df$nuts_year
                        ),
                        output_file = current_filename,
                        output_dir = "_lau_nuts_concordance_by_geo_html")
    }
  })
